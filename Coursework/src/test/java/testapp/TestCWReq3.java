...
public class TestCWReq3 extends TestCase {
    ...
    private Map<String, Integer> getExpected() throws SQLException {	
    	Map<String, Integer> totalNumber = new HashMap<String, Integer>();
    	String query = "select c.name, count(f.film_id) counter\n"
    			+ "from category c left join film_category f\n"
    			+ "on f.category_id = c.category_id\n"
    			+ "group by c.category_id, c.name";
    	
    	ResultSet rs = r.getResultSet(query);
    	//select c.name, count(f.film_id) counter from category c left join film_category f on f.category_id = c.category_id group by c.category_id, c.name
    	while(rs.next()) {
    		String name = rs.getString("name");
    		int counter = rs.getInt("counter");
    		totalNumber.put(name, counter);
    	}
    ...
    {
    	r.printOutput();
    	Map<String, Integer> actual = r.getActual();
